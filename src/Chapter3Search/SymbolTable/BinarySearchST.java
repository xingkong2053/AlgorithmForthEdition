package Chapter3Search.SymbolTable;
...
public class BinarySearchST<Key extends Comparable<Key>, Value> {
    ...
    /**
     * 二分查找实现算法Returns the number of keys in this symbol table strictly less than {@code key}.
     *
     * @param  key the key
    ...
    public int rank(Key key) {
        ...
        int lo = 0, hi = n-1;
        while (lo <= hi) {
            int mid = lo + (hi - lo) / 2;           //lo左边界，hi右边界，mid中间
            int cmp = key.compareTo(keys[mid]);
            if      (cmp < 0) hi = mid - 1;         //查找左半边
            else if (cmp > 0) lo = mid + 1;         //查找右半边
            else return mid;                        //命中
        }
        return lo;
    }
    ...
    public void put(Key key, Value val)  {
        ...
        for (int j = n; j > i; j--)  {  //将数组中在待插入位置后的所有元素先后移动一位（两个数组都要移动）
            keys[j] = keys[j-1];
            vals[j] = vals[j-1];
        }
        keys[i] = key;
